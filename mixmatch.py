...
def new_main():
    ...
    parser.add_argument('--num-labeled', type=int, default=2)
    parser.add_argument('--train-batch-size', default=8)
    parser.add_argument('--batch-size', default=256)

    parser.add_argument('--train-iteration', default=54, type=int)
    ...
    for i in range(args.ex_epochs):
        ...
        trainset, testset, valset, unlabelset = Data_Loader_MixMatch(args, path_list, train_transform=transform)
        ...
        #     model.zero_grad()
        MixMatch_train(args, train_loader, unlabel_loader, val_loader, model, optimizer, semi_criterion, ema_model,patience_check)
...
def MixMatch_train(args, train_loader, unlabeled_loader, test_loader, model, optimizer, criterion, ema_model, patience):
    global best_acc
    test_accs = []
    end = time.time()
